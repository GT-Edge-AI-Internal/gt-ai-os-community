...
class Settings(BaseSettings):
    ...
    # Idle timeout: 4 hours (enterprise-friendly, matches SessionService.IDLE_TIMEOUT_MINUTES)
    # Server-side session is authoritative, but JWT exp is used as fallback
    JWT_EXPIRES_MINUTES: int = Field(default=240, env="JWT_EXPIRES_MINUTES")
    # Absolute timeout: 8 hours (full workday) - forces re-authentication regardless of activity
    JWT_ABSOLUTE_TIMEOUT_HOURS: int = Field(default=8, env="JWT_ABSOLUTE_TIMEOUT_HOURS")
